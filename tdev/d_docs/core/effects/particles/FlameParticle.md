...
| 属性 | 值 |
|------|-----|
| **文件路径** | core/src/main/java/com/shatteredpixel/shatteredpixeldungeon/effects/particles/FlameParticle.java |
...
### 核心职责
实现火焰效果的视觉粒子表现，用于渲染火焰、燃烧等场景中的橙色上升发光粒子动画。
...
### 使用者
- 火焰相关的视觉效果场景（如燃烧地形、火焰陷阱等）
- 通过 `Emitter` 类使用 `FlameParticle.FACTORY` 发射粒子
...
### 创建时机
当需要显示火焰效果时，由 `Emitter` 调用 `FACTORY.emit()` 创建。
...
游戏事件（如燃烧地形）
    → 创建 Emitter
    → Emitter.pour(FlameParticle.FACTORY, ...)
    → FACTORY.emit() → reset()
    → 每帧调用 update()
    → 粒子消失后自动 kill()
```
...
emitter.pos(x, y);
emitter.pour(FlameParticle.FACTORY, 0.02f);  // 持续发射
scene.add(emitter);
```
...
### 重构建议
如需支持不同颜色的火焰效果（如蓝色火焰），可考虑添加颜色参数或创建子类。
