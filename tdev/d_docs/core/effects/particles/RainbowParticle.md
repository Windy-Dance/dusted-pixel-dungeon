...
| 属性 | 值 |
|------|-----|
| **文件路径** | core/src/main/java/com/shatteredpixel/shatteredpixeldungeon/effects/particles/RainbowParticle.java |
...
### 核心职责
实现彩虹效果的视觉粒子表现，用于渲染彩虹、庆祝等场景中的随机彩色粒子动画。
...
### 使用者
- 彩虹相关的视觉效果场景
- 通过 `Emitter` 类使用 `RainbowParticle.BURST` 发射粒子
...
public RainbowParticle() {
    super();
    color( Random.Int( 0x1000000 ) );  // 随机颜色（0x000000 ~ 0xFFFFFF）
    lifespan = 0.5f;
}
```

### 初始化注意事项
- 粒子颜色为随机值，范围 `0x000000` 到 `0xFFFFFF`（完整RGB色彩空间）
- 存活时间固定为 0.5 秒
- 每个粒子实例的颜色在构造时随机确定
...
- 可通过修改构造器中的颜色生成逻辑改变颜色策略

## 9. 运行机制与调用链

### 创建时机
当需要显示彩虹效果时，由 `Emitter` 调用工厂方法创建。
...
游戏事件（如庆祝效果）
    → 创建 Emitter
    → Emitter.burst(RainbowParticle.BURST, ...)
    → BURST.emit() → resetBurst()
    → 每帧调用 update() 更新大小和透明度
    → 粒子消失后自动 kill()
```
...
emitter.pos(x, y);
RainbowParticle p = new RainbowParticle();
p.reset(x, y);
emitter.add(p);
scene.add(emitter);
```
...
Emitter emitter = new Emitter();
emitter.pos(x, y);
emitter.burst(RainbowParticle.BURST, 30);  // 发射30个随机颜色粒子
scene.add(emitter);
```
...
- 粒子颜色在回收后不会重新随机，可能导致颜色重复
- 如需每次发射都随机颜色，需要在 `resetBurst()` 中调用 `color()`

## 13. 修改建议与扩展点

### 适合扩展的位置
- `update()` 中的大小变化逻辑可自定义
- 可在 `resetBurst()` 中添加颜色重新随机逻辑
- 构造器中的颜色生成策略可调整
...
### 重构建议
如需每次 `reset` 都重新随机颜色，应在 `resetBurst()` 中调用 `color(Random.Int(0x1000000))`。

## 14. 事实核查清单

- [x] 是否已覆盖全部字段：是，仅 `BURST` 一个静态字段
