# CorrosionParticle 文档

## 1. 基本信息

| 属性 | 值 |
|------|-----|
| **文件路径** | core/src/main/java/com/shatteredpixel/shatteredpixeldungeon/effects/particles/CorrosionParticle.java |
...
### 核心职责
实现腐蚀效果的视觉粒子表现，用于渲染腐蚀法术、腐蚀陷阱等场景中的变色粒子动画。
...
### 使用者
- 腐蚀相关的视觉效果场景
- 通过 `Emitter` 类使用 `CorrosionParticle.MISSILE` 或 `CorrosionParticle.SPLASH` 发射粒子
...
| 常量名 | 类型 | 值 | 说明 |
|--------|------|-----|------|
| `MISSILE` | Emitter.Factory | 匿名实现 | 导弹式粒子工厂，调用 `resetMissile()`，`lightMode()` 返回 `false` |
| `SPLASH` | Emitter.Factory | 匿名实现 | 溅射式粒子工厂，调用 `resetSplash()`，`lightMode()` 返回 `false` |
...
- 透明度：从 1 渐变到 0
- 大小：从 1 渐变到 4（逐渐变大）
- 颜色：从 `0xAAAAAA`（灰色）渐变到 `0xFF8800`（橙色）
...
### 创建时机
当需要显示腐蚀效果时，由 `Emitter` 调用 `MISSILE.emit()` 或 `SPLASH.emit()` 创建。
...
游戏事件（如腐蚀法术）
    → 创建 Emitter
    → Emitter.burst(CorrosionParticle.SPLASH, ...)
...
emitter.pos(x, y);
emitter.pour(CorrosionParticle.MISSILE, 0.05f);
scene.add(emitter);
```
...
Emitter emitter = new Emitter();
emitter.pos(x, y);
emitter.burst(CorrosionParticle.SPLASH, 20);  // 发射20个粒子
scene.add(emitter);
```
