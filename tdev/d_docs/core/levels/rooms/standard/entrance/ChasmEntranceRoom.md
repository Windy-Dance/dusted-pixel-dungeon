# ChasmEntranceRoom 文档

## 1. 基本信息

| 属性 | 值 |
|------|-----|
| **文件路径** | D:\Develop\Workspace\DustedPixelDungeon\core\src\main\java\com\shatteredpixel\shatteredpixeldungeon\levels\rooms\standard\entrance\ChasmEntranceRoom.java |
| **包名** | com.shatteredpixel.shatteredpixeldungeon.levels.rooms.standard.entrance |
| **文件类型** | class |
| **继承关系** | extends ChasmRoom |
| **代码行数** | 85 |
| **所属模块** | core |
...
### 核心职责
ChasmEntranceRoom 类负责创建和绘制具有深渊（CHASM）特征的入口房间。这类房间使用随机生成的深渊图案，并在安全位置设置通向上一层的入口。

### 系统定位
作为 ChasmRoom 的特殊变体，ChasmEntranceRoom 在较深的地牢关卡（深度21+）中出现，为入口房间提供最复杂的深渊布局样式。
...
### 父类提供的能力
- `ChasmRoom` 提供的深渊图案生成逻辑（setupPatch, fillPatch with Terrain.CHASM）
- `PatchRoom` 提供的补丁系统支持
...
### 构造器
ChasmEntranceRoom 使用默认构造器，没有显式的构造方法。继承自父类的构造逻辑。

### 初始化块
无静态或实例初始化块。

### 初始化注意事项
- 依赖父类 ChasmRoom 的 patch 系统进行深渊图案生成
...
**核心实现逻辑**：
调用父类的 minHeight() 方法，并与7取最大值，确保深渊入口房间足够大以容纳深渊图案和入口。
...
**核心实现逻辑**：
调用父类的 minWidth() 方法，并与7取最大值，确保深渊入口房间足够宽。
...
**核心实现逻辑**：
返回固定的概率数组，使得深渊入口房间主要生成 normal 大小，较少生成 large 大小，从不生成 giant 大小。
...
**核心实现逻辑**：
1. 调用 super.paint(level) 执行父类的深渊绘制逻辑
2. 随机选择一个内部位置作为入口点（避开怪物位置）
3. 包含额外的安全检查：在罕见的小而拥挤情况下，确保至少有一个相邻位置不是深渊（CHASM）
4. 设置入口地形为 Terrain.ENTRANCE
5. 将入口周围8邻域设置为空地（Terrain.EMPTY）
6. 添加 REGULAR_ENTRANCE 类型的 LevelTransition

**边界情况**：
- 入口位置选择最多尝试30次正常位置（非CHASM且无怪物）
- 如果30次都失败（极罕见情况），会检查4邻域确保至少有一个非CHASM位置
- 确保避免在完全被深渊包围的位置设置入口
...
### 扩展入口
- 可以通过继承 ChasmEntranceRoom 并覆写 paint() 等方法来自定义行为

## 9. 运行机制与调用链

### 创建时机
- 在 LevelGenerator.generate() 方法中，当深度为21+且随机选择到 ChasmEntranceRoom 类型时创建
...
### 系统流程位置
1. EntranceRoom.createEntrance() 创建 ChasmEntranceRoom 实例
...
### 基本用法
```java
// ChasmEntranceRoom 通过 EntranceRoom.createEntrance() 自动创建
// 无需手动实例化
```
...
```java
// 自定义深渊入口房间
public class CustomChasmEntranceRoom extends ChasmEntranceRoom {
    @Override
...
### 常见陷阱
- 修改尺寸约束时要考虑深渊图案生成的最小需求
- 入口周围清理逻辑确保玩家可以正常移动到入口位置，避免掉入深渊
- 安全检查逻辑处理极罕见的异常情况
