# RuinsEntranceRoom 文档

## 1. 基本信息

| 属性 | 值 |
|------|-----|
| **文件路径** | D:\Develop\Workspace\DustedPixelDungeon\core\src\main\java\com\shatteredpixel\shatteredpixeldungeon\levels\rooms\standard\entrance\RuinsEntranceRoom.java |
| **包名** | com.shatteredpixel.shatteredpixeldungeon.levels.rooms.standard.entrance |
| **文件类型** | class |
| **继承关系** | extends RuinsRoom |
| **代码行数** | 84 |
| **所属模块** | core |
...
### 核心职责
RuinsEntranceRoom 类负责创建和绘制具有废墟布局的入口房间。这类房间使用随机生成的废墟补丁（包含墙壁和 REGION_DECO 地形），并在安全位置设置通向上一层的入口。

### 系统定位
作为 RuinsRoom 的特殊变体，RuinsEntranceRoom 在最深的地牢关卡（深度21+）中出现，为入口房间提供带有废墟风格的复杂布局样式。

### 不负责什么
- 不负责处理玩家与入口的交互逻辑（由 LevelTransition 系统处理）
- 不负责新手引导页面的放置（仅在深度1-2的简单入口房间中放置）
- 不负责废墟补丁的基础生成逻辑（由父类处理）
...
### 父类提供的能力
- `RuinsRoom` 提供的废墟生成逻辑（patch系统、墙壁/REGION_DECO混合）
- `PatchRoom` 提供的补丁系统支持
...
### 依赖的关键类
- `Level`: 关卡数据结构
- `Terrain`: 地形类型定义（WALL, REGION_DECO, EMPTY, ENTRANCE）
- `Painter`: 地形绘制工具
...
### 构造器
RuinsEntranceRoom 使用默认构造器，没有显式的构造方法。继承自父类的构造逻辑。

### 初始化块
无静态或实例初始化块。

### 初始化注意事项
- 强制最小尺寸为7x7以确保有足够的空间容纳废墟布局和入口
...
**核心实现逻辑**：
调用父类的 minWidth() 方法，并与7取最大值，确保房间足够宽以容纳废墟布局和入口。
...
**核心实现逻辑**：
返回固定的概率数组，使得废墟入口房间主要生成 normal 大小，较少生成 large 大小，从不生成 giant 大小。
...
**核心实现逻辑**：
1. 调用 super.paint(level) 执行父类的废墟绘制逻辑
2. 随机选择一个内部位置作为入口点（避开怪物位置）
3. 包含额外的安全检查：在罕见的小而拥挤情况下，确保至少有一个相邻位置不是墙壁且不是 REGION_DECO
4. 设置入口地形为 Terrain.ENTRANCE
5. 将入口周围8邻域设置为空地（Terrain.EMPTY）
6. 添加 REGULAR_ENTRANCE 类型的 LevelTransition

**边界情况**：
- 入口位置选择最多尝试30次正常位置（非WALL且无怪物）
- 如果30次都失败（极罕见情况），会检查4邻域确保至少有一个安全位置
- 入口周围清理确保玩家可以正常移动到入口位置
...
### 扩展入口
- 可以通过继承 RuinsEntranceRoom 并覆写 paint() 等方法来自定义行为

## 9. 运行机制与调用链

### 创建时机
- 在 LevelGenerator.generate() 方法中，当深度为21+且随机选择到 RuinsEntranceRoom 类型时创建
...
### 系统流程位置
1. EntranceRoom.createEntrance() 创建 RuinsEntranceRoom 实例
...
### 依赖的资源
- 无直接的纹理/图标/音效依赖
- 间接依赖 Terrain.ENTRANCE、Terrain.WALL、Terrain.REGION_DECO 相关的视觉表现
...
### 基本用法
```java
// RuinsEntranceRoom 通过 EntranceRoom.createEntrance() 自动创建
// 无需手动实例化
```
...
```java
// 自定义废墟入口房间
public class CustomRuinsEntranceRoom extends RuinsEntranceRoom {
    @Override
...
### 状态依赖
- 依赖父类的 patch 系统生成废墟布局
- 依赖 Level.map 的当前状态进行入口位置选择
- 依赖 PathFinder.NEIGHBOURS4/NEIGHBOURS8 的固定顺序
...
### 常见陷阱
- 入口位置选择必须避开 WALL 地形
- 安全检查逻辑需要同时避开 WALL 和 REGION_DECO 地形
- 入口周围清理确保玩家安全移动到入口位置
