# RegionDecoPatchEntranceRoom 文档

## 1. 基本信息

| 属性 | 值 |
|------|-----|
| **文件路径** | D:\Develop\Workspace\DustedPixelDungeon\core\src\main\java\com\shatteredpixel\shatteredpixeldungeon\levels\rooms\standard\entrance\RegionDecoPatchEntranceRoom.java |
| **包名** | com.shatteredpixel.shatteredpixeldungeon.levels.rooms.standard.entrance |
| **文件类型** | class |
| **继承关系** | extends RegionDecoPatchRoom |
| **代码行数** | 108 |
| **所属模块** | core |
...
### 核心职责
RegionDecoPatchEntranceRoom 类负责创建和绘制具有装饰性补丁布局的入口房间。这类房间使用随机生成的 REGION_DECO 装饰性地形补丁，并在安全位置设置通向上一层的入口，同时支持新手引导页面放置。

### 系统定位
作为 RegionDecoPatchRoom 的特殊变体，RegionDecoPatchEntranceRoom 在前几层的地牢关卡（深度1-5）中出现，为入口房间提供带有装饰性补丁的布局样式。

### 不负责什么
- 不负责处理玩家与入口的交互逻辑（由 LevelTransition 系统处理）
- 不负责房间的基础装饰性补丁生成逻辑（由父类处理）
...
- 入口位置选择（避开 REGION_DECO 地形，包含安全检查）
- 入口周围8邻域清理为空地
...
### 父类提供的能力
- `RegionDecoPatchRoom` 提供的装饰性补丁生成逻辑（setupPatch, fillPatch with Terrain.REGION_DECO）
- `PatchRoom` 提供的补丁系统支持
...
### 构造器
RegionDecoPatchEntranceRoom 使用默认构造器，没有显式的构造方法。继承自父类的构造逻辑。

### 初始化块
无静态或实例初始化块。

### 初始化注意事项
- 强制最小尺寸为7x7以确保有足够的空间容纳装饰性补丁和入口
...
**核心实现逻辑**：
调用父类的 minHeight() 方法，并与7取最大值，确保房间足够高以容纳装饰性补丁和入口。
...
**核心实现逻辑**：
调用父类的 minWidth() 方法，并与7取最大值，确保房间足够宽。
...
**核心实现逻辑**：
1. 调用 super.paint(level) 执行父类的装饰性补丁绘制逻辑
2. 随机选择一个内部位置作为入口点（避开怪物位置）
3. 包含额外的安全检查：在罕见的小而拥挤情况下，确保至少有一个相邻位置不是 REGION_DECO
...
**边界情况**：
- 入口位置选择最多尝试30次正常位置（非REGION_DECO且无怪物）
- 如果30次都失败（极罕见情况），会检查4邻域确保至少有一个非REGION_DECO位置
...
### 扩展入口
- 可以通过继承 RegionDecoPatchEntranceRoom 并覆写 paint() 等方法来自定义行为

## 9. 运行机制与调用链

### 创建时机
- 在 LevelGenerator.generate() 方法中，当深度为1-5且随机选择到 RegionDecoPatchEntranceRoom 类型时创建
...
### 被调用者
- RegionDecoPatchRoom.paint(): 父类绘制逻辑
...
### 系统流程位置
1. EntranceRoom.createEntrance() 创建 RegionDecoPatchEntranceRoom 实例
2. LevelGenerator 将房间加入关卡并建立连接
3. Level.paint() 调用 paint() 方法
4. paint() 执行装饰性补丁绘制、入口设置和新手引导页面放置
...
### 基本用法
```java
// RegionDecoPatchEntranceRoom 通过 EntranceRoom.createEntrance() 自动创建
// 无需手动实例化
```
...
```java
// 自定义装饰性补丁入口房间
public class CustomRegionDecoPatchEntranceRoom extends RegionDecoPatchEntranceRoom {
    @Override
...
### 常见陷阱
- 修改入口位置选择逻辑时要注意 REGION_DECO 地形的避让
