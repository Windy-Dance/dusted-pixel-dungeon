...
### 核心职责
`FrostTrap`（冰霜陷阱）负责触发后在较大范围内释放冷冻效果，是 `ChillingTrap` 的增强版本。

### 系统定位
陷阱系统中的大范围冰冻型陷阱。覆盖范围比 ChillingTrap 更大（2 格距离）。

### 不负责什么
- 不直接对角色造成伤害（由 Freezing Blob 机制处理）
- 不负责冰冻的具体效果

## 3. 结构总览

### 主要成员概览
- **初始化块**：设置 `color = WHITE`，`shape = STARS`
- **activate() 方法**：释放冷冻效果
...
| 键名 | 中文翻译 | 用途 |
|------|---------|------|
| `levels.traps.frosttrap.name` | 冰霜陷阱 | 陷阱名称 |
| `levels.traps.frosttrap.desc` | 被触发时，这个陷阱里的化学药剂会迅速冻结附近大范围的空气。 | 陷阱描述 |
...
```java
// 创建并放置冰霜陷阱
FrostTrap trap = new FrostTrap();
trap.set(position);
trap.hide();

// 触发后在 2 格范围内生成冷冻效果（强度 20）
```
...
### 状态依赖
- 使用 `PathFinder.buildDistanceMap()` 计算范围
- 冷冻 Blob 强度固定
