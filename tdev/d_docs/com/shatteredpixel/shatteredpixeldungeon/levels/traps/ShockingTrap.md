...
### 核心职责
`ShockingTrap`（电击陷阱）负责触发后在周围小范围内释放电能，对角色造成电击效果。

### 系统定位
陷阱系统中的小范围电击型陷阱。是 `StormTrap` 的基础版本。
...
### 主要成员概览
- **初始化块**：设置 `color = YELLOW`，`shape = DOTS`
- **activate() 方法**：释放电能效果
...
| 键名 | 中文翻译 | 用途 |
|------|---------|------|
| `levels.traps.shockingtrap.name` | 电击陷阱 | 陷阱名称 |
| `levels.traps.shockingtrap.desc` | 一种储存着大量电能的机关。触发这个陷阱会让它把能量释放出来在附近形成小范围的电能场。 | 陷阱描述 |
...
// 触发后在 3x3 范围内生成电能（强度 10）
```

## 12. 开发注意事项

### 与 StormTrap 的区别
| 特性 | ShockingTrap | StormTrap |
|------|--------------|-----------|
| 范围 | 3x3 邻域 | 2 格距离 |
| 电能强度 | 10 | 20 |
| 形状 | DOTS | STARS |
